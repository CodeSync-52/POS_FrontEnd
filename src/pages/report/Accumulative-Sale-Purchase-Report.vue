<template>
  ...
  import { IAccumulativeSalePurchaseReportData, IUserResponse } from 'src/interfaces';
...
import { GetAccumalativeSalePurchaseReport } from 'src/services/reports';
  ...
  async function convertArrayToPdfData(array: IAccumulativeSalePurchaseReportData[]) {
    ...
    const footerRow = [
      ...
      {
        text: calculateTotal('Price').toString(),
        bold: true,
        margin: [0, 5],
      },
      {
  ...
  async function downloadPdfData() {
    ...
    downloadPdf({
      ...
      pdfHeaders: headers,
      title: 'Accumulative-Sale-Purchase-Stock-Report',
    });
