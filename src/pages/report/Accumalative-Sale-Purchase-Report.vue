<template>
  ...
  >
    <span class="text-xl font-medium">Accumalative Sale Purchase Report</span>
    <download-pdf-excel
  ...
  >
    ...
    <q-input
      ...
      dense
    />
 <div class="font-bold text-[20px]">Report Type :</div>
    ...
    <div class="flex lg:justify-end sm:justify-center items-end gap-2">
      <q-btn
        ...
        icon="search"
        label="Search"
        @click="accumalativeSalePurchaseReport()"
      />
      <q-btn
        unelevated
        color=""
  ...
  <div class="py-4">
    <q-table
      ...
      :rows="reportList"
      align="left"
      :columns="accumalativesalepurchaseReportColumn"
      row-key="id"
      :rows-per-page-options="[0]"
      @request="accumalativeSalePurchaseReport()"
...
<script setup lang="ts">
  ...
  import { IAccumalativeSalePurchaseReportData, IUserResponse } from 'src/interfaces';
  import { GetUsers } from 'src/services';
  import { isPosError, IPdfHeaders, ITableItems, downloadPdf } from 'src/utils';
  import { accumalativesalepurchaseReportColumn } from 'src/utils/reports';
...
import { GetAccumalativeSalePurchaseReport } from 'src/services/reports';
  const $q = useQuasar();
  const isLoading = ref(false);
  const isLoader = ref(false);
  const timeStamp = Date.now();
  const reportList = ref<IAccumalativeSalePurchaseReportData[]>([]);
  ...
  const accumalativeSalePurchaseReport = async () => {
    isLoading.value = true;
    ...
    try {
      const res = await GetAccumalativeSalePurchaseReport({
  ...
  async function convertArrayToPdfData(array: IAccumalativeSalePurchaseReportData[]) {
    ...
    array.forEach((item: IAccumalativeSalePurchaseReportData) => {
      const row = [
        { text: item.article },
        {
  ...
  async function downloadPdfData() {
    ...
    const myFileName = `Accumalative-Sale-Purchase-Stock-Report-${moment(
    ...
    downloadPdf({
      ...
      pdfHeaders: headers,
      title: 'Accumalative-Sale-Purchase-Stock-Report',
    });
    isLoader.value = false;
  }

  const downloadCSVData = () => {
    const selectedColumnsData = accumalativesalepurchaseReportColumn.filter(
      (col) => col.name !== 'image'
    );
    ...
    const status = exportFile(
      `Accumalative-Sale-Purchase-Stock-Report-${moment(filterSearch?.value?.fromDate).format(
        'DD/MM/YYYY'
